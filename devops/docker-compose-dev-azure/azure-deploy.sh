...
resourcename="businesssystem"  # lower case, only
resourcegroup="businesssystem_rg"
...
# git clone https://github.com/apilogicserver/businesssystem.git
# cd businesssystem
...
az group create --name businesssystem_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group businesssystem_rg --sku S1 --is-linux
az container create --resource-group businesssystem_rg --name businesssystem --image apilogicserver/businesssystem:latest --dns-name-label businesssystem --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//businesssystem.westus.azurecontainer.io:5656'
