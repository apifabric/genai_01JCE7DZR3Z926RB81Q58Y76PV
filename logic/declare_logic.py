...
def declare_logic():
    ...
    # Derives Customer.balance as sum of Order.amount_total where order is not shipped.
    Rule.sum(derive=Customer.balance, as_sum_of=Order.amount_total, where=lambda row: row.date_shipped is None)

    # Derives Order.amount_total as sum of Item.amount.
    Rule.sum(derive=Order.amount_total, as_sum_of=Item.amount)

    # Calculates Item.amount using quantity multiplied by unit_price.
    Rule.formula(derive=Item.amount, as_expression=lambda row: row.quantity * row.unit_price)

    # Copies Item.unit_price from Product.unit_price.
    Rule.copy(derive=Item.unit_price, from_parent=Product.unit_price)

    # Enforces that Customer.balance does not exceed credit_limit.
